...
def test(args, io, cfg, HM):
    ...
    if cfg.TRAIN.MC_LEVEL == 5:
        ...
        models = []
        for i in range(5):
            models.append(PointNet2(cfg).to(device))
            models[i].level = i
            models[i].load_state_dict(torch.load("/home/lixk/liaoyq/Campus3D_v2/checkpoints/MC"+str(i)+"/models/model2.t7"))
            if len(cfg.DEVICES.GPU_ID) > 1:
                model[i] = nn.DataParallel(model, device_ids=cfg.DEVICES.GPU_ID)
