...
class BlockSampler(_BasicSampler):
    ...
    def _build_query(self):
        tree_file = self.dataset.search_tree_path
        if os.path.isfile(tree_file) and not self.rebuild_tree:
            q = BlockQuery(tree_file,
                            self.knn_module,
                            (self.box_size_x, self.box_size_y))
        else:
            print('Building the search tree at {}...'.format(tree_file))
            q = BlockQuery(self.dataset.points[:, :-1],
                            self.knn_module,
                            (self.box_size_x, self.box_size_y))
            q.save(tree_file)
        return q
